...
# Current-main semantic-cohesion policy. Exact permission IDs and exact DTO names
# are evidence, but not business ontology. Hard contradictions are explicit
# context/aggregate divergence; routine additions may vary operation-specific
# permissions, authentication mechanisms and DTO names inside one declared domain.
write("app/cmd/boundarycore/decision.go", r'''package boundarycore
...
func EvaluateAddition(request AdditionRequest, before, after contract.Manifest) (ServiceBoundaryDecision, error) {
    ...
    if err != nil { return ServiceBoundaryDecision{}, fmt.Errorf("boundary addition before: %w", err) }
    right, err := Inspect(after, request.Application)
    if err != nil { return ServiceBoundaryDecision{}, fmt.Errorf("boundary addition after: %w", err) }
    ...
    for i := range right.Fingerprint.Operations {
        if right.Fingerprint.Operations[i].Plan.OperationID == request.OperationID { candidate = &right.Fingerprint.Operations[i] }
    }
    if candidate == nil { return ServiceBoundaryDecision{}, fmt.Errorf("boundary addition: operation %s is absent from target application", request.OperationID) }
    ...
    if preservesBaseline(before, after, *candidate) { scope.Verdict = "same"; scope.Evidence = []string{"before:manifest", "after:manifest"} } else { scope.Verdict = "different"; scope.CounterEvidence = []string{"before:manifest", "after:manifest"} }
    ...
    comparisons := []struct {
        ...
        {"consistency", "transaction and composition boundary must match an existing peer", func(a,b OperationEvidence) bool { return a.Plan.Execution.Transaction == b.Plan.Execution.Transaction && a.Plan.Composition.Boundary == b.Plan.Composition.Boundary }},
        {"dependency", "Application requirements/capabilities and declared child-Operation dependencies must match an existing peer", func(a,b OperationEvidence) bool { return jsonEqual(left.Fingerprint.ApplicationRequires, right.Fingerprint.ApplicationRequires) && jsonEqual(left.Fingerprint.Capabilities, right.Fingerprint.Capabilities) && jsonEqual(a.Plan.Composition.RequiresOperations, b.Plan.Composition.RequiresOperations) }},
    ...
    for _, peer := range left.Fingerprint.Operations { witnesses[peer.Plan.OperationID] = true }
    for _, compare := range comparisons {
        dim := newDimension(compare.name, true, compare.reason)
        for _, peer := range left.Fingerprint.Operations {
            if compare.equal(*candidate, peer) {
                dim.Evidence = append(dim.Evidence, peerRef(peer)); dim.Verdict = "compatible"
            } else {
                dim.CounterEvidence = append(dim.CounterEvidence, peerRef(peer)); delete(witnesses, peer.Plan.OperationID)
            }
        }
        dim.Evidence = append(dim.Evidence, "after:candidate")
        if len(left.Fingerprint.Operations) == 0 { dim.Verdict = "same" }
    ...
    if len(left.Fingerprint.Operations) == 0 {
        ...
        for _, peer := range left.Fingerprint.Operations {
            if witnesses[peer.Plan.OperationID] { d.PeerWitnesses = append(d.PeerWitnesses, peer.Plan.OperationID); coupling.Evidence = append(coupling.Evidence, peerRef(peer)) } else { coupling.CounterEvidence = append(coupling.CounterEvidence, peerRef(peer)) }
        }
        if len(d.PeerWitnesses) > 0 { coupling.Verdict = "compatible" }
    }
    ...
    if candidate.Boundary == nil || scope.Verdict != "same" {
        ...
        for _, peer := range peers {
            if peer.Boundary == nil { unknownIntent = true; continue }
        ...
        default:
            ...
            for _, dim := range d.Dimensions {
                if dim.Critical && dim.Verdict != "same" && dim.Verdict != "compatible" { all = false }
            }
            if all { d.Outcome = ReuseExistingService }
        }
    }

    counter := map[string]bool{}
    for _, dim := range d.Dimensions { for _, id := range dim.CounterEvidence { counter[id] = true } }
    sort.Slice(d.Evidence, func(i,j int) bool { return d.Evidence[i].ID < d.Evidence[j].ID })
    for _, ref := range d.Evidence { if counter[ref.ID] { d.CounterEvidence = append(d.CounterEvidence, ref) } }
...
func RevalidateAddition(request AdditionRequest, before, after contract.Manifest, proof ServiceBoundaryDecision) error {
    ...
    if err != nil { return fmt.Errorf("%w: cannot recompute decision: %v", ErrStaleBoundaryProof, err) }
    if !jsonEqual(current, proof) { return fmt.Errorf("%w: decision differs from current canonical policy evaluation", ErrStaleBoundaryProof) }
    return nil
}

func newDimension(name string, critical bool, reason string) BoundaryDimension { return BoundaryDimension{Name:name, Verdict:"unknown", Critical:critical, Reason:reason, Evidence:[]string{}, CounterEvidence:[]string{}} }
...
func intentDimensionV2(name string, candidate OperationEvidence, peers []OperationEvidence) BoundaryDimension {
    ...
    if candidate.Boundary == nil { d.CounterEvidence = append(d.CounterEvidence, "after:candidate"); return d }
    if len(peers) == 0 { d.Verdict = "same"; d.Evidence = append(d.Evidence, "after:candidate"); return d }
    unknown, different := false, false
    for _, peer := range peers {
        if peer.Boundary == nil { unknown = true; d.CounterEvidence = append(d.CounterEvidence, peerRef(peer)); continue }
        same := candidate.Boundary.Context == peer.Boundary.Context
        if name == "aggregate" { same = aggregateKey(candidate.Boundary) == aggregateKey(peer.Boundary) }
        if same { d.Evidence = append(d.Evidence, peerRef(peer)) } else { different = true; d.CounterEvidence = append(d.CounterEvidence, peerRef(peer)) }
    }
    d.Evidence = append(d.Evidence, "after:candidate")
    if different { d.Verdict = "different" } else if !unknown { d.Verdict = "same" }
    return d
}

func aggregateKey(value *contract.BoundaryIntent) string {
    if value == nil { return "<unknown>" }
    if strings.TrimSpace(value.Aggregate) != "" { return "aggregate:" + strings.TrimSpace(value.Aggregate) }
...
func permissionFamilies(op OperationEvidence) []string {
    families := []string{}
    if op.Plan.Security.Public { families = append(families, "public") }
    for _, permission := range op.Plan.Security.Permissions {
        permission = strings.TrimSpace(permission)
        if permission == "" { continue }
        if index := strings.LastIndex(permission, "."); index > 0 { permission = permission[:index] }
        families = append(families, permission)
    }
    sort.Strings(families)
    result := families[:0]
    for _, value := range families { if len(result)==0 || result[len(result)-1] != value { result = append(result, value) } }
    return append([]string(nil), result...)
}

func sameClientContractFamily(a,b OperationEvidence) bool {
    if typePackage(a.Plan.RequestType) != typePackage(b.Plan.RequestType) || typePackage(a.Plan.ResponseType) != typePackage(b.Plan.ResponseType) { return false }
    if a.ClientStreaming != b.ClientStreaming || a.ServerStreaming != b.ServerStreaming { return false }
    if (a.Plan.Bindings.RPC == "") != (b.Plan.Bindings.RPC == "") { return false }
    return jsonEqual(httpMethodFamily(a), httpMethodFamily(b))
}
func typePackage(value string) string { if i := strings.LastIndex(strings.TrimSpace(value), "."); i > 0 { return value[:i] }; return strings.TrimSpace(value) }
func httpMethodFamily(op OperationEvidence) []string { values:=[]string{}; for _, h := range op.Plan.Bindings.HTTP { values=append(values, strings.ToUpper(strings.TrimSpace(h.Method))) }; sort.Strings(values); return values }

func hasOperation(m contract.Manifest, id string) bool {
    for _, s := range m.Services {
        for _, method := range s.Methods { if method.Operation != nil && method.Operation.ID == id { return true } }
        if s.Application != nil { for _, op := range s.Application.Operations { if op.ID == id { return true } } }
    }
    return false
}
func detachedManifest(m contract.Manifest) contract.Manifest { data,_:=json.Marshal(m); var copy contract.Manifest; _=json.Unmarshal(data,&copy); copy.Normalize(); return copy }
func modelDigest(domain string, v any) string { data,_:=json.Marshal(v); h:=sha256.Sum256(append([]byte(domain+"\n"),data...)); return hex.EncodeToString(h[:]) }
func jsonEqual(a,b any) bool { x,ex:=json.Marshal(a); y,ey:=json.Marshal(b); return ex==nil && ey==nil && string(x)==string(y) }
func fullCommitSHA(s string) bool { if len(s)!=40 && len(s)!=64 { return false }; if strings.Trim(s,"0")=="" { return false }; for _,c:=range s { if !((c>='0'&&c<='9')||(c>='a'&&c<='f')) { return false } }; return true }
func containsString(values []string, v string) bool { for _,item:=range values { if item==v { return true } }; return false }
''')

# Pressure fixture: use the real repository DSL include while authoring, and give
# both established Operations explicit coherent boundary intent. Their operation-
# specific permission IDs and DTO names stay distinct to exercise v2 cohesion.
replace_once(
...
# Create-plan helpers now carry the same explicit boundary and compiler include.
replace_once(
...
# ChangeSet reconciliation helper must use the same exact include profile/boundary.
for old, new in [
    ('changeSetCreateOptions(fixture.Root, true)', 'changeSetCreateOptions(fixture, true)'),
    ('changeSetCreateOptions(fixture.Root, false)', 'changeSetCreateOptions(fixture, false)'),
]:
    text = read("app/cmd/change/changeset_reconcile_test.go")
    write("app/cmd/change/changeset_reconcile_test.go", text.replace(old, new))
replace_once(
...
# T4/Biz create fixtures remain intentionally different in permission/auth details;
# they only declare the same architectural boundary and actual compiler include.
text = read("app/cmd/change/changeset_biz_regression_test.go")
text = text.replace('apiKeyArchiveOptions(fixture.Root)', 'apiKeyArchiveOptions(fixture)')
text = text.replace('func apiKeyArchiveOptions(root string) add.OperationOptions {', 'func apiKeyArchiveOptions(fixture pressureFixture) add.OperationOptions {')
text = text.replace('Root: root, ApplicationKey: "tenant/lifecycle", OperationID: "tenant.archive"', 'Root: fixture.Root, ApplicationKey: "tenant/lifecycle", OperationID: "tenant.archive"')
text = text.replace('Authentication: []string{"api-key"}, Transaction: "local", Idempotency: "none", Composition: "local",\n\t}', 'Authentication: []string{"api-key"}, Transaction: "local", Idempotency: "none", Composition: "local",\n\t\tBoundaryContext: "tenant.lifecycle", BoundaryAggregate: "tenant", ProtoPaths: []string{fixture.ProtoPath},\n\t}')
# Inline service-auth option.
text = text.replace('Authentication: []string{"service", "jwt", "api-key"}, Transaction: "local", Idempotency: "none", Composition: "local",\n\t}', 'Authentication: []string{"service", "jwt", "api-key"}, Transaction: "local", Idempotency: "none", Composition: "local",\n\t\tBoundaryContext: "tenant.lifecycle", BoundaryAggregate: "tenant", ProtoPaths: []string{fixture.ProtoPath},\n\t}')
write("app/cmd/change/changeset_biz_regression_test.go", text)

# Source-scope create case is about source authority, not missing architecture intent.
# Give its existing peer and candidate the same explicit boundary; keep distinct DTO
# names/permissions behavior available to the v2 evaluator.
replace_once(
    "app/cmd/change/contract_sources_test.go",
    'execution: {transaction: TRANSACTION_LOCAL idempotency: IDEMPOTENCY_NONE} };',
    'execution: {transaction: TRANSACTION_LOCAL idempotency: IDEMPOTENCY_NONE} boundary: { context: "scope.lifecycle" aggregate: "scope" } };')
replace_once(
    "app/cmd/change/contract_sources_test.go",
...
print("semantic boundary v2 and fixture alignment prepared")
