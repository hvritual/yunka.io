...
import (
	...
	"yunka-rpc/protobuf/pkg/typemap"
	"yunka-rpc/protobuf/pkg/utils"
)
...
type Base struct {
	...
	ProjectName         string // Name of the project that we're generating
	FrameworkName       string // Name of the frame that we're generating
...
func (t *Base) Setup(in *plugin.CodeGeneratorRequest, paramsOpt ...GeneratorParamsInterface) {
	...
	var genPkgName = t.Params.ImportMap["package"]
	if genPkgName == `` {
	...
	t.ProjectName = t.Params.ImportMap["project"]
	if t.ProjectName == `` {
		t.ProjectName = `xirang`
	}


	t.FrameworkName = t.Params.ImportMap["framework"]
	if t.FrameworkName == `` {
		t.FrameworkName = `xirang`
	}
