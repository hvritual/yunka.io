...
func TestIssue161SetBoundaryRecomputesForgedProof(t *testing.T) {
	...
	cases := []struct {
		...
		{"non-reuse", func(c *CreateOperationChange) {
			c.BoundaryProof.Plan.BoundaryDecision.Outcome = boundarycore.ArchitectureReviewRequired
		}},
		...
		{"before-fingerprint", func(c *CreateOperationChange) {
			c.BoundaryProof.Plan.BoundaryDecision.BeforeFingerprint = strings.Repeat("d", 64)
		}},
		{"candidate-plan", func(c *CreateOperationChange) {
			c.BoundaryProof.Plan.BoundaryDecision.OperationPlanDigest = strings.Repeat("e", 64)
		}},
		{"omitted-counter-evidence", func(c *CreateOperationChange) { c.BoundaryProof.Plan.BoundaryDecision.CounterEvidence = nil }},
		{"altered-dimension", func(c *CreateOperationChange) {
			c.BoundaryProof.Plan.BoundaryDecision.Dimensions[0].Reason = "invented supporting evidence"
		}},
	}
...
func TestIssue161SetBoundaryRejectsIgnoredBaselineDriftAndCancellation(t *testing.T) {
	fixture := newPressureFixture(t)
	writePressureFile(t, filepath.Join(fixture.Root, ".yunka", "project.json"), `{"version":2,"database":{"tablePrefix":"yk"},"workflow":{"contract":{"protoRoot":"contracts/proto","generated":"contracts/generated"},"modules":{"root":"modules"},"generatedGo":{"root":"internal"},"dev":{"manifest":".yunka/dev.json"}}}`)
