...
func GenerateWithOptions(options Options) error {
	...
	files := map[string]string{
		...
		filepath.Join("autoload", "register.go"): autoloadTemplate(normalized.Name, packageImport),
	}
	order := []string{
		"config.go",
		"dependencies.go",
...
func moduleTemplate(options Options) string {
	...
	for _, name := range options.RPC {
		checks = append(checks, fmt.Sprintf(`if dependencies.%s == nil {
		return nil, fmt.Errorf("%%s: RPC target %s is required", ModuleName, err)
	}`, rpcField(name), name))
	}
	var builder strings.Builder
	fmt.Fprintf(&builder, "package %s\n\n", options.Name)
...
func autoloadTemplate(moduleName, packageImport string) string {
...
const (
	GeneratedModuleName = %q
	GeneratedModuleImportPath = %q
	GeneratedDescriptorSymbol = "GeneratedDescriptor"
)

func init() { modulecatalog.MustRegister(module.GeneratedDescriptor()) }
`, packageImport, moduleName, packageImport)
}
