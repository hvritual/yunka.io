...
var (
	_map = map[string]apiTranslateHandle{
		...
		T_UNION: func(api *APIStruct, tag, value string) bool {
			...
			if err != nil {
				log.Error("union:", err, value, api.Uri)
				return true
			}
...
func parseParam(uri, tagName string, parent *APIItem, filedName string, objField *types.Struct) []APIItem {
	...
	hasCnt := structMap[key]
	if hasCnt+1 > maxDeep {
		log.Error(key)
		return params
	}

	structMap[key] = hasCnt + 1
...
func parseFile(uri, tagName string, fileAndField string) []APIItem {
	...
	if !strings.Contains(fileAndField[:pkgIdx], "request") &&
		!strings.Contains(fileAndField[:pkgIdx], "bean") {
		pkgIdx = index
	}
	...
	pkg, err := config.Check("", fset, fs, info)
	if err != nil {
	...
	if _type == nil {
		log.Fatal("can't find ", fileAndField)
	}
