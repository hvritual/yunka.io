...
- C10.4 is complete on the stacked framework/consumer branches: the real Biz Device Transfer runtime regenerates through C10 assembly with second-generation zero drift, standard verification and MySQL 8.4 pressure green, and migrated product code no longer hand-registers canonical REST/gRPC or constructs generated child capabilities. This qualification does not imply merge to `main`; C10.5 remains the release/merge gate.
- When a consumer's typed `ApplicationFactories` or canonical `operation.Executor` requires capabilities owned by the same App's prepared `platform.Provider`, generated assembly may use the C10.4 `RuntimeBinder` seam. The binder runs only after `kernel.New` has prepared Platform, returns only typed Factories + Executor, and must not prepare/recreate Platform, discover services, own lifecycle, or introduce parallel authorization/transaction semantics.
