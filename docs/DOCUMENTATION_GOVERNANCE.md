...
- a pressure item changes between hypothesis, open, deferred, blocking, or closed in a way that affects current planning;
...
`PROJECT_MEMORY.md` is the durable decision memory. It should contain:
...
It must not contain independently changing current-state facts such as current HEAD, repository visibility, active PR/task state, a current wave table, or a duplicated pressure ledger. Those facts belong in `docs/STATUS.md`, Git/GitHub state, or exact evidence records.
