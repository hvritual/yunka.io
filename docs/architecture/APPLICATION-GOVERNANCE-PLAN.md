...
AG-05 的命令和单一 Go-native 后端契约见 [APPLICATION-SOURCE-POLICY.md](APPLICATION-SOURCE-POLICY.md)：完整物理清点、显式模块/工作区/替换及构建矩阵、目录组件依赖与生产/测试资产隔离，均与分析完成度分开。既有依赖版本政策不复制、不放宽；生成注释不构成豁免。持久只读资格工作流对所有 Go 文件变更触发，保留框架与固定消费者的真实正反例。真实旧模块的不完整结果不冒充消费者已合规；后续模板、债务和批量改造仍为独立任务。
