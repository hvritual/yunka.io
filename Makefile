...
dsl-check: toolchain-check architecture-check rpc-tools rpc-toolchain-check
	@set -eu; protoc_path="$$(command -v "$(PROTOC)")"; cd pkg && YUNKA_REQUIRE_C84_RUNTIME=1 YUNKA_REQUIRE_C86_RUNTIME=1 YUNKA_REQUIRE_C87_RUNTIME=1 YUNKA_REQUIRE_C9_RUNTIME=1 PROTOC="$$protoc_path" PROTOC_GEN_GO="$(PROTOC_GEN_GO)" PROTOC_GEN_GO_GRPC="$(PROTOC_GEN_GO_GRPC)" $(GO) test -count=1 ./contract -run '^(TestC8(4GeneratedApplication|6GeneratedMultiApplication|7GeneratedComposition)Runtime|TestC9Generated(UnifiedExecution|CrossDomainAcyclicApplicationGraph)Runtime)$$'
...
c10-3-runtime-closure-check: toolchain-check architecture-check rpc-tools rpc-toolchain-check
	@set -eu; protoc_path="$$(command -v "$(PROTOC)")"; cd app && YUNKA_REQUIRE_C103_QUALIFICATION=1 PROTOC="$$protoc_path" PROTOC_GEN_GO="$(PROTOC_GEN_GO)" PROTOC_GEN_GO_GRPC="$(PROTOC_GEN_GO_GRPC)" $(GO) test -timeout=10m -count=1 ./cmd/assembly -run '^TestC103QualificationFullAssembledRuntimeClosure$$'
